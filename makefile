...
unzip: $(addprefix data/unzipped/,$(addsuffix /,$(shell ls data/raw | sed 's/\.tar\.gz//g' | sed 's/_/\//g')))

data/unzipped/LT/%: data/raw/%
	tar -xf $< -C $(dir $@)

data/unzipped/.%.done:
	$(RUN) python3 papers_past/convert_xml.py --xml_dir `echo data/unzipped/$* | sed 's/_/\//g'`
	touch $@

clean:
	rm -rf data/unzipped/* data/unzipped/.*
